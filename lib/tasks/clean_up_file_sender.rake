namespace :share_file do
  ...
  task :clean_up_file_senders => :environment do
    ...
    ShareFilesApp::FileSender.where("expiry_date < '#{Time.now}'").destroy_all
    broken_uplaods = ShareFilesApp::Upload.where("uploaded_size != file_size and updated_at < '#{Time.now + 1.hours}'")
